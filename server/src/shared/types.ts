...
export interface Recipe {
  id: string;
  name: string;
  ingredients: { type: IngredientType; state: ItemState }[];
  points: number;
...
export interface ServerToClientEvents {
  ...
  'room-joined': (data: { room: RoomInfo; playerId: string }) => void;
  ...
  'game-over': (data: { score: number; won: boolean }) => void;
...
// DTO for sending game state over network (Map converted to object)
...
export interface StationConfig {
  type: StationType;
  position: Position;
  ingredientType?: IngredientType; // For ingredient boxes
}
